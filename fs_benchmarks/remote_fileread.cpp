...
struct timediff t;
const char* file_path = "/home/abaksy/mountpoint/temp_file";
...
{
    int SAMPLES = 3;
    long long int sizes[] = {16*MB, 32*MB, 64*MB, 128*MB, 256*MB, 512*MB, 1*GB};
    ...
    for (int i = 0; i < SIZES; i++) {
        ...
        for (int j = 0; j < SAMPLES; j++) {
            ...
            //File read access 
            long double time_taken = 0; // sequential_reads(fd, bytesForFile, buffer);
        ...
        long double average_cycles = (double)totaltime / (SAMPLES);
